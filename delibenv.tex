...
\commentOCf{“we claim that discussions are valuable to the extent that they allow fostering rationality and anti-paternalism” ⇒ pas seulement. Comme discuté au téléphone, la discussion pourrait être valuable pour mettre le DM dans le bon état d’ouverture d’esprit lorsqu’il écoute les arguments. De même, la phrase \og{}if rationality and anti-paternalism can be achieved without the kind of informal talks that one calls “discussions” in everyday life, then these “discussions” will be dispensable\fg{} me semble trop forte, puisque les discussions pourraient avoir un autre intérêt. On pourrait réduire notre ambition et dire qu’on veut clarifier le “rôle de la délibération” issue, et non pas indiquer clairement quel est son rôle ; en proposant que son rôle est au moins en partie de s’assurer de la rationalité et anti-paternalisme, mais sans prendre position sur d’autres utilités qu’elle pourrait avoir ? Ou, de façon intéressante, on pourrait aller plus loin en indiquant une troisième utilité de la délibération, celle de s’assurer qu’on a mis le DM dans \og{}le bon\fg{} état d’esprit. Je suis en faveur de cette dernière piste.}\commentYM{la première piste me semble effectivemenent plus prudente que la formuation adoptée, par contre je ne comprends pas comment on pourrait emprunter la seconde piste, dans la mesure où le formalisme ne formalise pas ce que la troisième piste ajoute}
...
We are interested in defining the \ac{DM}’s stance towards the topic once a deliberation has allowed her to ponder all the arguments possibly relevant to the situation, so has to reach her pondered (rational) own (anti-paternalistic) stance. We call this her \ac{DJ}.

This is done by assuming that there is a protocol available, that permits to observe the reactions of the \ac{DM} to the arguments. The protocol permits to observe two type of things. First, whether the \ac{DM} considers that a given argument trumps some other given argument, meaning that the second argument makes the first one invalid for some of its claims. Second, whether the \ac{DM} considers that some argument leads to consider the propositions that the argument defends as valid. 

Importantly, the postulated protocol tolerates changes of mind of the \ac{DM}, as we can assume that this individual might initially be observed as considering that some argument trumps some other one, then, upon being presented a third argument that the \ac{DM} considers trumps the second one, will no longer declare that the second argument trumps the first one. 

The concepts defined so far permit to define \emph{decisive} arguments, which in turn permit to define the \ac{DJ} of the \ac{DM}. An argument is decisive if the \ac{DM} never considers that any argument trumps it. Then, the \ac{DJ} of the \ac{DM} is simply the set of propositions in the topic that are supported by decisive arguments.
Those propositions are called acceptable. This definition echoes \possessivecite{rawls_political_2005} emphasis on the requirement of \emph{acceptability} by reasonable citizens. 
...
\Citet{cailloux_formal_2019} also prove that, if a series of conditions are fulfilled, an operationally valid model is valid. (The proof being technical, we cannot summarize it here, but refer the interested reader to that article.)
...
\commentOCf{Closed under reinstatement pose un problème. J’aimerais présenter juste l’idée grossièrement. Qqch. comme suit. Closed under reinstatement, similarly, introduces some “coherence” constraints on the way the \ac{DM} reacts to some patterns of arguments trumping each other. Voire même, ne rien en dire et indiquer que les trois conditions sont un peu techniques, tout comme la preuve du théorème, et qu’on les saute donc pour la même raison, et donner uniquement l’explication ci-dessus de Justifiable I. comme exemple. Ça fonctionne bien si on enlève la discussion en conclusion en 3.2. Elle ne me semble pas essentielle, pas immédiatement liée à notre propos, et répète ce qu’on a écrit par ailleurs, je crois. On pourrait alors remplacer 3.2 par insister sur la question de l’étude empirique de l’effet du contexte (délibération en présence par exemple) sur l’état d’esprit du décideur, qui est plus liée à notre propos dans cet article. (On pourrait aussi mettre ces conditions en annexe mais j’aime nettement moins cette option.)}\commentYM{oui, je suis assez d'accord. Le contenu technique des conditions n'est pas le sujet ici. Le message important est de montrer que les choses ne sotn pas triviales, qu'il faut des critères opérationels, et que des conditions non triviales sont nécessaires pour qu'on puisse passer de ce qu'on observe à ce qu'il nous intéresse de pouvoir en conclure. Tout cela demande un peu d'explication, mais pas du contenu technique des conditions ni du théorème}
...
\Citet{cailloux_formal_2019} proved that when these three conditions are met, there exists an operationally valid model, and any operationally valid model is valid. 
This theorem provides an operational way of testing a model’s claims, and thus, checking whether \thedm’s \ac{DJ} has been captured.
%\begin{example}[cont.]
